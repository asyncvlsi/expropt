...
private:
    ...
    /**
     * the map that maps the expr pointers as long int to the names that are printed in the optimised act, leaf variabes only
     */
    ...
    /**
     * the map that maps the expr pointers as long int to the names that are printed in the optimised act, output expr/ports only
     */
     ...
     */
    void search_var_in_expr ( list_t *var_list, Expr *e);
     ...
     */
    void search_expr (act_chp_lang_t *c);

    /**
     * if you encounter a sequential operation in the chp data structure use this mehtod to flush out a set op expression that are paralell.
     * it will also call the EXPROPT lib for running the syntesis
     */
    void start_new_set ();
};
