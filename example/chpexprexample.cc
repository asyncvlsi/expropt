 ...
 */
chpexprexample::chpexprexample(char *out, expr_mapping_software software, Act *act_instance,
...
void chpexprexample::start_new_set() {
    ...
    for (li = list_first(out_expr_bundle); li; li = list_next(li)) {
        search_var_in_expr(in_expr_bundle, (const Expr *)list_value(li));
    }
    ExprBlockInfo *info;
    // run the syntesis
    info = optimiser->run_external_opt(expr_set_name, in_expr_bundle, inexprmap, inwidthmap, out_expr_bundle,
                                       outexprmap, outwidthmap);

    printf("Generated block %s: Area: %e m2, Power: %e W, delay: %e s, max power: %e, min delay: %e, max delay: %e, "
           "static power: %e, dynamic power: %e (if 0 => circuit empty, extraction failed or corner not provided)\n",
           expr_set_name, info->area, info->power_typ, info->delay_typ, info->power_max, info->delay_min,
           info->delay_max, info->power_typ_static, info->power_typ_dynamic);
 ...
 */
void chpexprexample::search_var_in_expr(list_t *in_bundle, const Expr *e) {
    switch (e->type) {
    case (E_PROBE):
        fatal_error("probes are not working yet");
        break;
 ...
 */
void chpexprexample::search_expr(const act_chp_lang_t *c) {
    //  int expr_width;
    //  InstType *it;
    ...
    case ACT_CHP_COMMA: {
        ...
        for (li = list_first(c->u.semi_comma.cmd); li; li = list_next(li)) {
            const act_chp_lang_t *cmd = (const act_chp_lang_t *)list_value(li);
            search_expr(cmd);
        }
    } break;
    // loops and selects can have all guards bundeled but need sequential exec for the satements so they have to go in
    // seperate blocks
    case ACT_CHP_SELECT:
    case ACT_CHP_LOOP: {
