...
ExprBlockInfo *ExternalExprOpt::run_external_opt(int expr_set_number, int targetwidth, const Expr *expr,
    ...
    std::unordered_map<const Expr *, const char *> outexprmap;
    std::unordered_map<const Expr *, const char *> inexprmap;
    std::unordered_map<const Expr *, int> outwidthmap;
    std::list<const Expr *> outlist;

    // convert input list, reverse searching nessesary, should always use last on multimatching
    for (const Expr *e : in_expr_list) {
...
ExprBlockInfo *ExternalExprOpt::run_external_opt(int expr_set_number, int targetwidth, const Expr *expr,
    ...
    return run_external_opt(expr_set_number, targetwidth, expr, to_list<const Expr *>(in_expr_list),
                            to_imap<const Expr *, int>(in_expr_map), to_imap<const Expr *, int>(in_width_map));
}
 ...
 * ExprOpt standart
 */
ExprBlockInfo *ExternalExprOpt::run_external_opt(int expr_set_number, const std::list<std::pair<int, int>> &in_list,
...
ExprBlockInfo *ExternalExprOpt::run_external_opt(int expr_set_number, list_t * /*expr_list*/, list_t *in_list,
    ...
    return run_external_opt(expr_set_number, to_int_pair_list(in_list), to_int_pair_list(out_list),
...
ExprBlockInfo *ExternalExprOpt::run_external_opt(const char *expr_set_name, list_t *in_expr_list,
    ...
    return run_external_opt(expr_set_name, to_list<const Expr *>(in_expr_list),
                            to_map<const Expr *, const char *>(in_expr_map), to_imap<const Expr *, int>(in_width_map),
                            to_list<const Expr *>(out_expr_list), to_map<const Expr *, const char *>(out_expr_map),
                            to_imap<const Expr *, int>(out_width_map), to_list<const Expr *>(hidden_expr_list));
}
ExprBlockInfo *ExternalExprOpt::run_external_opt(const char *expr_set_name, const std::list<const Expr *> &in_expr_list,
    ...
    for (const Expr *li : out_expr_list) {
        out_name_list.push_back(out_expr_map.at(li));
    }
    for (const Expr *li : hidden_expr_list) {
...
ExprBlockInfo *ExternalExprOpt::run_external_opt(const char *expr_set_name, list_t *in_expr_list,
    ...
    return run_external_opt(expr_set_name, to_list<const Expr *>(in_expr_list),
                            to_map<const Expr *, const char *>(in_expr_map), to_imap<const Expr *, int>(in_width_map),
                            to_list<const Expr *>(out_expr_list), to_list<const char *>(out_expr_name_list),
                            to_imap<const Expr *, int>(out_width_map), to_list<const Expr *>(hidden_expr_list),
                            to_list<const char *>(hidden_expr_name_list));
}
ExprBlockInfo *ExternalExprOpt::run_external_opt(const char *expr_set_name, const std::list<const Expr *> &in_expr_list,
                                                 const std::unordered_map<const Expr *, const char *> &in_expr_map,
                                                 const std::unordered_map<const Expr *, int> &in_width_map,
    ...
    print_expr_verilog(verilog_stream, expr_set_name, in_expr_list, in_expr_map, in_width_map, out_expr_list,
                       out_expr_name_list, out_width_map, hidden_expr_list, hidden_expr_name_list);
 ...
 */
void ExternalExprOpt::print_expr_verilog(
    FILE *output_stream, const char *expr_set_name, const std::list<const Expr *> &in_list,
    const std::unordered_map<const Expr *, const char *> &inexprmap,
    const std::unordered_map<const Expr *, int> &inwidthmap, const std::list<const Expr *> &out_list,
    const std::list<const char *> &out_expr_name_list, const std::unordered_map<const Expr *, int> &outwidthmap,
    const std::list<const Expr *> &expr_list, const std::list<const char *> &hidden_expr_name_list) {
    if (!output_stream)
 ...
 */
void ExternalExprOpt::print_expression(FILE *output_stream, const Expr *e,
...
void ExternalExprOpt::generate_genus_tcl(const char *tcl_file_name, const char *file_name, const char *out_file_name,
    ...
    if (std::strcmp(configreturn, "none") != 0)
        fprintf(tcl_file, "set_db init_lib_search_path \"%s\"\n", configreturn);
    ...
    if (std::strcmp(configreturn, "none") != 0)
        fprintf(tcl_file, "set_db library_domain:tt_normaltemp .library { %s } \n", configreturn);
    else
        fatal_error("please define \"liberty_tt_typtemp\" in expropt configuration file");
    ...
    if (std::strcmp(configreturn3, "none") != 0) {
        ...
        if (std::strcmp(configreturn, "none") != 0 && std::strcmp(configreturn2, "none") != 0 && configint != 0) {
            ...
            fprintf(tcl_file, "create_delay_corner -name CORNER_TIMING_SLOW -timing_condition TIMING_COND_SLOW "
                              "-rc_corner RC_CORNER_RCMAX_HIGHTEMP\n");
            fprintf(tcl_file, "create_analysis_view -name ANALYSIS_VIEW_TIMING_SLOW -constraint_mode CONSTRAINT_MODE_1 "
                              "-delay_corner CORNER_TIMING_SLOW\n");
            delay_max = true;
        }
        ...
        if (std::strcmp(configreturn, "none") != 0 && std::strcmp(configreturn2, "none") != 0 && configint != 0) {
            ...
            fprintf(tcl_file, "create_rc_corner -name RC_CORNER_RCMIN_LOWTEMP -temperature %d -qrc_tech %s\n",
                    configint, configreturn2);
            fprintf(tcl_file, "create_delay_corner -name CORNER_TIMING_FAST -timing_condition TIMING_COND_FAST "
                              "-rc_corner RC_CORNER_RCMIN_LOWTEMP\n");
            fprintf(tcl_file, "create_analysis_view -name ANALYSIS_VIEW_TIMING_FAST -constraint_mode CONSTRAINT_MODE_1 "
                              "-delay_corner CORNER_TIMING_FAST\n");
            delay_min = true;
        }
        ...
        if (std::strcmp(configreturn, "none") != 0 && std::strcmp(configreturn2, "none") != 0 && configint != 0) {
            ...
            fprintf(tcl_file, "create_rc_corner -name RC_CORNER_RCTYP_TYPTEMP -temperature %d -qrc_tech %s\n",
                    configint, configreturn2);
            fprintf(tcl_file, "create_delay_corner -name CORNER_TYP -timing_condition TIMING_COND_TYP -rc_corner "
                              "RC_CORNER_RCTYP_TYPTEMP\n");
            fprintf(tcl_file, "create_analysis_view -name ANALYSIS_VIEW_TYP -constraint_mode CONSTRAINT_MODE_1 "
        ...
        if (std::strcmp(configreturn, "none") != 0 && std::strcmp(configreturn2, "none") != 0 && configint != 0) {
            ...
            fprintf(tcl_file, "create_rc_corner -name RC_CORNER_RCMIN_MAXTEMP -temperature %d -qrc_tech %s\n",
                    configint, configreturn2);
            fprintf(tcl_file, "create_delay_corner -name CORNER_POWER_MAX -timing_condition TIMING_COND_MAX_POWER "
                              "-rc_corner RC_CORNER_RCMIN_MAXTEMP\n");
            fprintf(tcl_file, "create_analysis_view -name ANALYSIS_VIEW_POWER_MAX -constraint_mode CONSTRAINT_MODE_1 "
                              "-delay_corner CORNER_POWER_MAX\n");
            power_max = true;
        }
        ...
        if (delay_min || typ || delay_max || power_max) {
            ...
            if (typ)
                fprintf(tcl_file, " ANALYSIS_VIEW_TYP");
            if (delay_min)
                fprintf(tcl_file, " ANALYSIS_VIEW_TIMING_FAST");
            fprintf(tcl_file, " }");
            ...
            if (typ)
                fprintf(tcl_file, " ANALYSIS_VIEW_TYP");
            if (delay_max)
                fprintf(tcl_file, " ANALYSIS_VIEW_TIMING_SLOW");
            if (delay_min)
                fprintf(tcl_file, " ANALYSIS_VIEW_TIMING_FAST");
            if (power_max)
                fprintf(tcl_file, " ANALYSIS_VIEW_POWER_MAX");
            fprintf(tcl_file, " }");

            if (typ)
                fprintf(tcl_file, " -leakage ANALYSIS_VIEW_TYP -dynamic ANALYSIS_VIEW_TYP");
            else if (power_max)
                fprintf(tcl_file, " -leakage ANALYSIS_VIEW_POWER_MAX -dynamic ANALYSIS_VIEW_POWER_MAX");
            fprintf(tcl_file, "\n");
        }
    }
    ...
    if (typ)
        fprintf(tcl_file, "redirect %s.timing_typ.log { report_timing -unconstrained -view ANALYSIS_VIEW_TYP }\n",
                out_file_name);
    else
        fprintf(tcl_file, "redirect %s.timing_typ.log { report_timing -unconstrained }\n", out_file_name);
    if (delay_min)
        fprintf(tcl_file,
                "redirect %s.timing_min.log { report_timing -unconstrained -view ANALYSIS_VIEW_TIMING_FAST }\n",
                out_file_name);
    if (delay_max)
        fprintf(tcl_file,
                "redirect %s.timing_max.log { report_timing -unconstrained -view ANALYSIS_VIEW_TIMING_SLOW }\n",
                out_file_name);
    if (typ)
        fprintf(tcl_file, "redirect %s.power_typ.log { report_power -view ANALYSIS_VIEW_TYP }\n", out_file_name);
    else
        fprintf(tcl_file, "redirect %s.power_typ.log { report_power }\n", out_file_name);
    if (power_max)
        fprintf(tcl_file, "redirect %s.power_typ.log { report_power -view ANALYSIS_VIEW_POWER_MAX }\n", out_file_name);
...
double ExternalExprOpt::parse_and_return_max(std::string filename, std::string parse_format, double failure_value,
                                             bool fail_if_file_does_no_exist) {
